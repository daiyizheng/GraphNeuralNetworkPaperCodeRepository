...
def sample_edge_uniform(n_triples, sample_size):
    ...
    all_edges = np.arange(n_triples) # 所有节点递增编号
    return np.random.choice(all_edges, sample_size, replace=False) #按编号抽样sample_size个数据

def negative_sampling(pos_samples, num_entity, negative_rate):
    size_of_batch = len(pos_samples) # 每个batch阳性例子   num_entity entity个数 negative_rate采样比例
    num_to_generate = size_of_batch * negative_rate
    neg_samples = np.tile(pos_samples, (negative_rate, 1))# np.tile沿着Y轴平铺negative_rate个
...
def edge_normalization(edge_type, edge_index, num_entity, num_relation):
    ...
    deg = scatter_add(one_hot, edge_index[0], dim = 0, dim_size = num_entity) # edge_index[0] 为 src->node 索引，  rel的个数求和。 one_hot()
...
def generate_sampled_graph_and_labels(triplets, sample_size, split_size, num_entity, num_rels, negative_rate):
    ...
    edges = sample_edge_uniform(len(triplets), sample_size) # 在triplets中采样数据

    # Select sampled edges
    edges = triplets[edges]# 根据编号索引得到采样的边数据
    src, rel, dst = edges.transpose()
    uniq_entity, edges = np.unique((src, dst), return_inverse=True)# edges 是旧列表在新列表uniq_entity中的位置索引
    src, dst = np.reshape(edges, (2, -1))# 转成[2, xx]
    relabeled_edges = np.stack((src, rel, dst)).transpose()

    # Negative sampling
    samples, labels = negative_sampling(relabeled_edges, len(uniq_entity), negative_rate) # 负采样
    ...
    data.entity = torch.from_numpy(uniq_entity)# 唯一采样节点
